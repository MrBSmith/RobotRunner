...
var save_directories = GameSaver.find_all_saves_directories(GAME.SAVEGAME_DIR)
...
func _ready():
	...
	for i in range(0,3):
		var save_name : String = GameSaver.get_save_cfg_property_value_by_name_and_cfgid(GAME.SAVEGAME_DIR, "save_name", i+1)
		if save_name == "":
...
func update_save_information(slot_id : int):
	if !save_directories.empty():
		...
		if slot_id == -1:
			...
			var target_cfg_save_time = GameSaver.get_save_cfg_property_value_by_name_and_cfgid(GAME.SAVEGAME_DIR, "time",slot_id)
			if typeof(target_cfg_save_time) == TYPE_DICTIONARY:
				load_save_name_info_label_node.text = "Name : " + GameSaver.find_corresponding_save_file(GAME.SAVEGAME_DIR, slot_id)
				...
				load_save_xion_info_label_node.text = "Xion : " + str(GameSaver.get_save_cfg_property_value_by_name_and_cfgid(GAME.SAVEGAME_DIR, "xion", slot_id))
				load_save_gear_info_label_node.text = "Gear : " + str(GameSaver.get_save_cfg_property_value_by_name_and_cfgid(GAME.SAVEGAME_DIR, "gear", slot_id))
	else:
		$VBoxContainer.visible = false

func load_save(slot_id : int):
	var save_path : String = str(GameSaver.load_settings(GAME.SAVEGAME_DIR, slot_id))
...
func _on_menu_option_focus_changed(_button : Button, focus: bool) -> void:
	...
	var target_save_time = GameSaver.get_save_cfg_property_value_by_name_and_cfgid(GAME.SAVEGAME_DIR, "time", buttonindex)
